...
static void tracking_notify_cb (GSettings *settings, const gchar *key, ZoomOptionsPrivate *priv);
...
init_mouse_mode_radio_group (GSList *mode_group, ZoomOptionsPrivate *priv)
{
    ...
    g_signal_connect (G_OBJECT (mode_group->data), "toggled",
                      G_CALLBACK(tracking_radio_toggled_cb),
                      priv);
    }
}

static void
init_focus_mode_radio_group (GSList *mode_group, ZoomOptionsPrivate *priv)
{
    gchar *mode;
    gchar *name;

    mode = g_settings_get_string (priv->settings, "focus-tracking");
    for (; mode_group != NULL; mode_group = mode_group->next)
      {
        name = (gchar *) gtk_buildable_get_name (GTK_BUILDABLE (mode_group->data));
        if (g_strcmp0 (name, mode) == 0)
...
static void
init_caret_mode_radio_group (GSList *mode_group, ZoomOptionsPrivate *priv)
{
    gchar *mode;
    gchar *name;

    mode = g_settings_get_string (priv->settings, "caret-tracking");
    for (; mode_group != NULL; mode_group = mode_group->next)
      {
        name = (gchar *) gtk_buildable_get_name (GTK_BUILDABLE (mode_group->data));
        if (g_strcmp0 (name, mode) == 0)
...
static void
tracking_notify_cb (GSettings             *settings,
                          const gchar           *key,
