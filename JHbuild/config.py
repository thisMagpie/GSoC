...
class Config:
    ...
    def setup_env(self):
        ...
        # GJS_TYPELIB_PATH
        if not 'GJS_TYPELIB_PATH' in os.environ:
            if self.use_lib64:
                full_name = '/usr/lib64/gjs'
            else:
                full_name = '/usr/lib/gjs'
            if os.path.exists(full_name):
                addpath('GJS_TYPELIB_PATH', full_name)
        typelibpath = os.path.join(self.libdir, 'gjs')
        addpath('GJS_TYPELIB_PATH', typelibpath)
